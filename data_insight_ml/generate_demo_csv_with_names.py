"""
Generate Demo CSV Files with IDs and Names for Upload Testing
Creates realistic CSV files with identifiable information
"""
...
# Sample names for different purposes
...
org_names = ['Hope Foundation', 'Community Aid Network', 'Future Leaders', 'Bright Tomorrow', 'Unity Project',
             'Green Valley Initiative', 'Safe Haven', 'Education First', 'Health Access', 'Youth Empowerment',
             'Family Support Center', 'Development Alliance', 'Care Coalition', 'Vision Fund', 'Impact Hub']

# 1. Donor Retention Demo CSV
print("Generating Donor Retention demo CSV...")
donor_data = {
    'donor_id': [f'DNR{str(i).zfill(4)}' for i in range(1001, 1016)],
    'donor_name': [f'{np.random.choice(first_names)} {np.random.choice(last_names)}' for _ in range(15)],
...
program_data = {
    'participant_id': [f'PRT{str(i).zfill(4)}' for i in range(2001, 2016)],
    'participant_name': [f'{np.random.choice(first_names)} {np.random.choice(last_names)}' for _ in range(15)],
...
grant_data = {
    'application_id': [f'GRT{str(i).zfill(4)}' for i in range(3001, 3016)],
    'organization_name': [np.random.choice(org_names) for _ in range(15)],
...
churn_data = {
    'customer_id': [f'MBR{str(i).zfill(4)}' for i in range(4001, 4016)],
    'member_name': [f'{np.random.choice(first_names)} {np.random.choice(last_names)}' for _ in range(15)],
...
student_data = {
    'student_id': [f'STU{str(i).zfill(4)}' for i in range(5001, 5016)],
    'student_name': [f'{np.random.choice(first_names)} {np.random.choice(last_names)}' for _ in range(15)],
...
child_data = {
    'child_id': [f'CHD{str(i).zfill(4)}' for i in range(6001, 6016)],
    'child_name': [f'{np.random.choice(first_names)} {np.random.choice(last_names)}' for _ in range(15)],
...
print("\n" + "="*60)
print("All demo CSV files with IDs and names created successfully!")
print("="*60)
print("\nFiles created:")
print("  1. demo_upload_donor_retention.csv (with donor_id, donor_name)")
print("  2. demo_upload_program_completion.csv (with participant_id, participant_name)")
print("  3. demo_upload_grant_scoring.csv (with application_id, organization_name)")
print("  4. demo_upload_customer_churn.csv (with customer_id, member_name)")
print("  5. demo_upload_student_dropout.csv (with student_id, student_name)")
print("  6. demo_upload_child_wellbeing.csv (with child_id, child_name)")
print("\nEach file contains 15 sample records with realistic IDs and names!")
print("Ready to upload through the demo.html interface.")

# Print sample preview
print("\n" + "="*60)
print("SAMPLE PREVIEW - Student Dropout CSV (first 3 rows):")
print("="*60)
print(student_df.head(3).to_string(index=False))
print("\n" + "="*60)
print("SAMPLE PREVIEW - Child Wellbeing CSV (first 3 rows):")
print("="*60)
print(child_df.head(3).to_string(index=False))
